...
| **M4** | Memory, hybrid retrieval, retrieval eval set. Vision via ScreenCast/PipeWire. | M2, M3. | Retrieval measured against a fixed eval set; brute-force-vs-ANN decision made by benchmark, not preference. | XL, 25–35 ideal-days. | Not started. See `docs/architecture/m4-memory-retrieval.md` (placeholder, still gate-only). `docs/architecture/m4-scoping-notes.md` has real research and a scope-decision question list prepared ahead of this milestone's own real planning pass -- not a design, not an ADR, gated on the user's own answers, same discipline M3 followed before its own real design was drafted. |

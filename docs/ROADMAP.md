...
| **M4** | Memory, hybrid retrieval, retrieval eval set. Vision via ScreenCast/PipeWire. | M2, M3. | Retrieval measured against a fixed eval set; brute-force-vs-ANN decision made by benchmark, not preference. | XL, 25–35 ideal-days (not re-estimated after scoping -- see below). | **Scoping complete, design drafted -- implementation not started.** The user answered `docs/architecture/m4-scoping-notes.md`'s five open questions (2026-08-25); `docs/architecture/m4-memory-retrieval.md` is now a real design (no longer a placeholder), and ADR-0048 through ADR-0052 are drafted, **Proposed, not yet Accepted**. Real, direct consequence of the scoping answers, not yet acted on outside `m4-memory-retrieval.md` itself: vision moves to M5 in full (this row's own Objective column still says "Vision via ScreenCast/PipeWire" -- a real edit this row and M5's own row both still need, flagged rather than silently made as a side effect of this status update). No code exists yet; WP-57 through WP-64 are sketched at objective-level in `m4-memory-retrieval.md`'s own "Work package sketch" section, none started. |

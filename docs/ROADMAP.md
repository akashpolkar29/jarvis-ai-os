...
| **M6a** | Communications/productivity: email via IMAP (read)/SMTP (send), calendar via CalDAV (vendor-neutral throughout, ADR-0021), research (resolved: no new port, reuses M5's existing `BrowserAutomationPort` unmodified). | M5. | Per-plugin conformance to the M0 capability/policy/audit model. | Not specified. | **Read half real, tested, and implemented (WP-76 through WP-80); write half (send/create) entirely unbuilt, explicitly blocked on ADR-0057.** `docs/architecture/m6a-communications.md` (2026-09-01) is the real design; ADR-0057 (email-send/attended-calendar-event classification) remains **Proposed, not Accepted** — unreviewed by the user directly. Every capability whose classification depends on that ADR (`EmailPort.send_message`, `CalendarPort.create_event`) is real, present on its own Protocol, and unconditionally raises `NotImplementedError` in every real adapter — proven structurally, not just by inspection (`tests/meta/test_communications_no_send_or_create.py`, mirroring M6b's own "prove the boundary" meta-test precedent; bans `smtplib` and `caldav`'s own real, confirmed-current event-creation method names). **`communications.list_email`/`communications.read_email`/`communications.list_calendar_events`** are real, invocable, `Effect.EGRESS_LOCAL`/`Tier.ALLOW` capabilities — `ports/email.py`/`ports/calendar.py`, real `ImapEmailAdapter`/`CalDavCalendarAdapter` (stdlib `imaplib` for IMAP; the real `caldav` library, newly added, for CalDAV — its own real API quirks, confirmed via `mypy --strict`'s own `reveal_type`, not assumed, are documented in the adapter's own module docstring), `kernel/communications.py`. Real, adversary-influenced content is tainted per-item at the point it enters this codebase, mirroring `browser.screenshot`'s own precedent. **A real, deliberately conservative scoping choice beyond what the design doc itself required**: even attendee-less `create_event` (which `m6a-communications.md` classifies as ordinary `WRITE_LOCAL`/`CONFIRM`, not gated by ADR-0057 at all) is left unimplemented in this pass, so there is exactly one clean, unambiguous "everything past reading needs ADR-0057" boundary rather than a partially-implemented write path. `email_port`/`calendar_port` have no default (mirror `job_assistance.draft`'s own `providers` precedent) — real per-deployment IMAP/CalDAV configuration is not this module's decision. |
...
- **M6a**: [`docs/architecture/m6a-communications.md`](architecture/m6a-communications.md) —
  ...
  process — but, like M5's own design doc, resting on classification
  reasoning worked through remotely, not confirmed by the user
  directly in this pass (see the document's own header). One new ADR,
  `0057`, **Proposed, not Accepted**.
- **M6b**: [`docs/architecture/m6-integrations.md`](architecture/m6-integrations.md) —
  still a placeholder; real scoping is done

...
| **M6a** | Communications/productivity: email via IMAP (read)/SMTP (send), calendar via CalDAV (vendor-neutral throughout, ADR-0021), research (resolved: no new port, reuses M5's existing `BrowserAutomationPort` unmodified). | M5. | Per-plugin conformance to the M0 capability/policy/audit model. | Not specified. | **Real design done, not yet implemented.** `docs/architecture/m6a-communications.md` (2026-09-01) — real `EmailPort`/`CalendarPort` shapes, a real classification decision (`Effect.EGRESS_SENSITIVE`/`Effect.EGRESS_SECRET`, reused rather than a new effect — ADR-0057, **Proposed, not Accepted**), a real acceptance-criteria list, and a work-package sketch (WP-76 through WP-81). No `ports/email.py`/`ports/calendar.py`/`adapters/email.py`/`adapters/calendar.py`/`kernel/communications.py` exist yet — design and ADR-drafting only, matching M3/M4/M5's own "design precedes implementation by a separate pass" discipline. |
| **M6b** | Job assistance: research and drafting only, no auto-apply — **resolved 2026-09-02, directly by the user**: "no auto-apply" is a structural boundary, not a policy-tier gate (see `docs/adr/0058-m6b-no-auto-apply-is-a-structural-boundary-not-a-policy-tier-gate.md`, **Accepted**). | M5. | Per-plugin conformance to the M0 capability/policy/audit model — plus `m6b-job-assistance.md`'s own six real acceptance criteria. | Not specified. | **Code-complete** (WP-82 through WP-85), all gates pass — see `docs/architecture/m6b-job-assistance.md`, ADR-0058, and `docs/threat-model/v0.md`'s "Milestone 6b additions" for what was actually built and verified. The structural meta-test (`tests/meta/test_job_assistance_no_submission.py`) landed first, proven against an empty package, before any real capability code existed — mirroring this project's own "safety-critical piece first" discipline. `job_assistance.draft` reuses M2's existing `UnverifiableTaskHandler` (not `Dispatcher` — checked and rejected, no pass/fail validator exists for prose), one new, minimal `DraftStoragePort`/`LocalDraftStorageAdapter` (with a real, defensive path-traversal-sanitization finding made during implementation, not named in the design doc itself), and no outer `coding.run_task`-style gate (checked and rejected as unnecessary — no per-write inner loop exists here). Research needed no new code at all — checked against M6a's own item-6 resolution, not assumed to match. **A real, conservative implementation default, flagged for the user's own confirmation, not silently decided**: `SECRET`-classified drafting input reuses `Effect.MEMORY_WRITE`'s own unconditional-`DENY` floor — this also makes `job_assistance.draft` a dynamic-effect capability, a real deviation from the design doc's original static-capability sketch, forced by implementing the classification function for real. **Real, explicitly-named gaps, none rounded up to "done"**: no real caller (CLI or voice) invokes `authorize_and_draft_document` yet; the SECRET-input default is implemented and tested but not yet user-confirmed as the right long-term choice; the meta-test's own real, named limits (it cannot inspect provider-generated content for adversarial intent, its identifier ban-list is a fixed set not an exhaustive network-reachability proof, and its scan boundary is M6b's own package path only); no console-line wiring; which real job-listing source(s) a future research flow would use remains open. **Not tagged** — tagging remains a separate, later action. |

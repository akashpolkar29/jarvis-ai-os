...
| **M6b** | Job assistance: research and drafting only, no auto-apply — **resolved 2026-09-02, directly by the user**: "no auto-apply" is a structural boundary, not a policy-tier gate (see `docs/adr/0058-m6b-no-auto-apply-is-a-structural-boundary-not-a-policy-tier-gate.md`). | M5. | Per-plugin conformance to the M0 capability/policy/audit model — plus `m6b-job-assistance.md`'s own six real acceptance criteria. | Not specified. | **Real design done** (`docs/architecture/m6b-job-assistance.md`, 2026-09-02) — research resolves to no new port (reuses M5's `BrowserAutomationPort`, the same conclusion M6a's own item 6 reached, checked separately); drafting reuses M2's existing `UnverifiableTaskHandler` (not `Dispatcher` — checked and rejected, no pass/fail validator exists for prose) plus one new, minimal `DraftStoragePort`; the structural no-submission boundary is enforced by a real, specified (not yet implemented) AST-based meta-test, not policy alone. ADR-0058 drafted, **Proposed, not Accepted** — its core Decision is the user's own, but the document's own written text still needs their direct review. **Not yet implemented** — no `ports/draft_storage.py`, `adapters/draft_storage.py`, `application/job_assistance/`, or `kernel/job_assistance.py` exist; `m6b-job-assistance.md`'s own work-package sketch (WP-82 through WP-86) is planning only. |

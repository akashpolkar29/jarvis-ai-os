...
# not implemented early, since neither package (per-adapter
# subpackages, a real plugins/* member) exists on disk yet. C5 (ui
# privilege) was one such estimate too, until WP-24 made jarvis.ui real
# and configured it for real below, which is why C5's number was
# corrected down to when that concretely happened rather than left at
# its original guess. C4 in particular is very likely to become due
# during M2 (WP-28+): ADR-0038 through ADR-0041 record decisions for
# application/reasoning/ and adapters/reasoning/, adapters/validation/
# -- the first real per-adapter subpackages in this repo, and exactly
# the scenario C4 was written for. Whichever work package actually
# creates those adapter subpackages must configure C4 in pyproject.toml
# and correct this schedule's "30" to the real WP number in the same
# change, not leave it to this test's failure to notice.

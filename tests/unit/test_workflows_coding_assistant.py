"""Unit tests for the real, built-in Coding Assistant workflow (WP-187).

All three real ``Tier.ALLOW`` steps are mocked at their real
``kernel.capability_dispatch`` dispatch point, exactly matching
``test_workflows_research.py``'s own established discipline -- this
avoids touching a real git repository or scanning the real
...
def test_coding_assistant_is_registered_with_four_real_steps() -> None:
    """The real, built-in registry contains this workflow, with its four documented steps."""
    workflow = build_default_workflow_registry().get(CODING_ASSISTANT_WORKFLOW_ID)

    assert len(workflow.steps) == _EXPECTED_STEP_COUNT
    assert workflow.steps[0].capability_id == CapabilityId("git.status")
    assert workflow.steps[1].capability_id == CapabilityId("fs.find")
    assert workflow.steps[2].capability_id == CapabilityId("fs.search_content")
    assert workflow.steps[3].capability_id == CapabilityId("coding.run_task")


def test_denied_outer_gate_never_inspects_searches_or_runs_the_coding_agent(
    tmp_path: Path,
) -> None:
...
def test_granted_run_inspects_and_searches_then_halts_before_the_coding_agent(
    tmp_path: Path,
) -> None:
    """The three ALLOW steps run for real; coding.run_task (CONFIRM) halts, never auto-invoked."""
    with (

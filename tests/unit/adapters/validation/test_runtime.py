"""Unit tests for jarvis.adapters.validation.runtime.RuntimeCheckValidator.
...
from jarvis.adapters.validation.runtime import RuntimeCheckValidator
...
async def test_validate_passes_when_the_patch_applies_and_the_program_exits_zero(
    tmp_path: Path,
) -> None:
    (tmp_path / "hello.txt").write_text("original\n", encoding="utf-8")
    validator = RuntimeCheckValidator(
    ...
    assert evidence[0].author == "runtime"


async def test_validate_fails_when_the_program_exits_nonzero(tmp_path: Path) -> None:
    (tmp_path / "hello.txt").write_text("original\n", encoding="utf-8")
    validator = RuntimeCheckValidator(

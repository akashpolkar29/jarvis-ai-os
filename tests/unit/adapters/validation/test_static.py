"""Unit tests for jarvis.adapters.validation.static.StaticAnalysisValidator.
...
from jarvis.adapters.validation.static import StaticAnalysisValidator
...
async def test_validate_passes_when_the_patch_applies_and_analysis_exits_zero(
    tmp_path: Path,
) -> None:
    ...
    assert evidence[0].author == "static"


async def test_validate_fails_when_analysis_exits_nonzero(tmp_path: Path) -> None:
    (tmp_path / "hello.txt").write_text("original\n", encoding="utf-8")
    validator = StaticAnalysisValidator(

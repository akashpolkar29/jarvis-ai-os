...
## Current Status
M0 complete, tagged v0.1.1. M1 (voice) is code-complete, tagged v0.2.0 — all 7 gates pass, CI green — see `docs/architecture/m1-voice-architecture.md`. Live end-to-end voice verification is still an open item (tracker #19): the bug is isolated to the capture code's own audio path, not the OS/PipeWire layer. Next work package: M2 planning (WP-28) — see `docs/architecture/m2-reasoning-layer.md`, recovered but not yet re-validated against what M0 became.
